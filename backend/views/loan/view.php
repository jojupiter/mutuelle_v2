...
$loans = (new Query())->from('loan')->where(['id_session' => $sessions_exercise, 'matricule_member'=>$matricule_member])->orderBy('id')->all();
$this->title = $member->firstname.' '.$member->lastname.' - '.$month[$exercice->month].' '.$exercice->year;
$this->params['breadcrumbs'][] = ['label' => 'Loans', 'url' => ['index', 'id'=>$exercise_id]];
    ...
    <div class="col-12" style="margin: 0 auto;">
            <div class="card" id="changeable" style="width: 70%; margin: 0 auto;">
                <div class="card-header" style="text-align: center;">
                    <h3 class="card-title">History of <?= $member->firstname.' '.$member->lastname ?> loans for the exercise <?= $month[$exercice->month].' '.$exercice->year ?></h3>
                </div>
                <!-- /.card-header -->
                <div class="card-body">
                    <table class="table table-bordered table-striped" style="width: 100%; margin: 0 auto; vertical-align: middle;" id="example">
                        ...
                        <tbody style="vertical-align: middle;">
                           <?php foreach($loans as  $loan): ?>
                            <?php $session = Session::findOne(['id' => $loan['id_session']]); ?>
                            <tr>
                                <td><?= date('d F, Y (l)', strtotime($session['date_'])); ?></td>
                               <td style="text-align: center;"><?= number_format($loan['amount'],2,",",".") ?></td>
                            </tr>
                            <?php endforeach; ?> 
                        </tbody>
                    </table>
